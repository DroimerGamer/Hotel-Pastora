...
class inputformProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Form::macro('UTPassword', function ($password,$label,$title,$value=null, $errors,$length,$required=null,$readonly=null,$click=null)
        {
            if($errors->has($password)) {$comp = '<div class="form-group has-error">';} else {$comp = '<div class="form-group form-float">';}
            if($required != null){$atributes['required']='';}
            $atributes['class']='form-control';
            $atributes['onkeypress']='return UTPassword(event,this);';
            $atributes['title']=$title;
            $atributes['maxlength']='10';
            $atributes['style']='background-color: #ffffff;';
            if($click!=null){$atributes['onclick']=$click;}
            if($readonly!=null){$atributes['readonly']=$readonly;}
            $comp.= '<div class="form-line">';
            if($value!=null){$comp.= Form::password($password, array( 'class' =>$password));}else{$comp.=  Form::password($password, array( 'class' => 'form-control'));}
            if($required){$comp.= Form::toHtmlString("<label class='form-label'>".ucfirst($label)."<span class=\"required\" style='color:red!important;'>&nbsp;<strong>*</strong></span></label>");}else{$comp .= Form::label($password, $label, array('class'=>'form-label'));}
            $comp.= '</div>';
            if($errors->has($password)) {$comp .= '<span class="help-block">'.$errors->first($password).'</span>';}
            $comp .= '</div>';
            return $comp;
        });
        ...
        Form::macro('UTList', function ($name,$label,$title,$options,$default = -1,$errors,$required=null,$onchange=null,$click=null)
        {
            ...
            $atributes['class']='form-control';
            if($click!=null){$atributes['onclick']=$click;}
            $comp.= '<div class="col-lg-12 col-md-12 col-sm-6 col-xs-12"><div class="form-line" style="border: none!important;">';
            ...
            $comp.= '</div></div><div class="col-lg-12 col-md-12 col-sm-6 col-xs-12">';
        ...
        Form::macro('UTCalendar', function ($name, $label,$title,$value=null, $errors, $required=null)
        {
            ...
            if($required){$comp.= Form::toHtmlString("<label class='form-label'>".ucfirst($label)."<span class=\"required\" style='color:red!important;'>&nbsp;<strong>*</strong></span></label>");}else{$comp .= Form::label($name, $label, array('class'=>'form-label'));}
            $comp.= '<div class="form-line">';
            if($value != null){$comp.=Form::date($name, $value);}else{$comp.=Form::date($name, \Carbon\Carbon::now());}
            $comp.= '</div>';
            if($errors->has($name)) {$comp .= '<span class="help-block">'.$errors->first($name).'</span>';}
            $comp .= '</div>';
            return $comp;
